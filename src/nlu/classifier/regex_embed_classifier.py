...
# from regex_intent_classification import Intent_Classification
...
class RegexEmbeddingIntentClassifier(Component):
    ...
    def __init__(
        ...
        self._load_params()
        # self.intent_classify = Intent_Classification()
    ...
    def process(self, message: "Message", **kwargs: Any) -> None:
        ...
        # print (label)
        if self.config.get('use_classifi_nlu') == False:
            label_regex = self.intent_classify.predict(message.text) #get intent which used regex to classification
            label_real = label
            if label_regex!= None:
                for label in label_ranking:
                    if label_regex == label["name"]:
                        label["confidence"] = 1.0
                        label_real = label
            message.set("intent", label_real, add_to_output=True)
